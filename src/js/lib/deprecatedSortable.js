    ...
    module.directive('svRoot', [function(){
        ...
        return {
            ...
            controller: ['$scope', '$attrs', '$interpolate', '$parse', function($scope, $attrs, $interpolate, $parse){
                ...
                // $interpolate is either taking markup with {{}} or just some other string and evaluating it in some context (like $scope)
                // $interpolate uses $parse internally. think of it as a simple templating language like Jade
                // http://stackoverflow.com/questions/17900588/what-is-the-difference-between-the-parse-interpolate-and-compile-services
                // $interpolate is read only.
                // $parse is individual expressions only, like $scope.name or $scope.id;
                // youre supposed to assign soem kind of ID onto sv-root attribute
                ...
                var isGrid       = false;
                // $parse turns angular expression from the scope into a function. so $attrs.svOnSort would be something like  'ViewCtrl.doThisAfterSort' and $parse($attrs.svOnSort) turns into a function into which you can
                // pass in a context
                var onSort       = $parse($attrs.svOnSort);

                // ----- hack due to https://github.com/angular/angular.js/issues/8044
                // an issue where angualr replaces the "start" or "stop" with "" and not allowing you to use any attribute names containing those.
                // $$element is used internally. it points to the directive's element.
                // $$element.attributes has all the attributes on the element markup
                ...
                // if svOnStop is defined, then $attrs.svOnStop assign the value on the markup. normally this will be an expression that evaluates to a function
                $attrs.svOnStop = $attrs.svOnStop && $attrs.svOnStop.value;
                // -------------------------------------------------------------------

                // now we are getting the actual on-start and on-stop functions from the scope/context
                ...
                if($attrs.svGrid){ // sv-grid determined explicite
                    ...
                    $scope.$watchCollection(function(){
                        ...
                        // turns a collection of items [{part:id, y:ypos},{part:id, y:ypos},{part:id, y:ypos}...] into a map {id:[ypos], id:[ypos], ....}
                        ...
                        // figures out whether the items are a grid or not by comparing each element's "top" position to the next element's "top" in the list (if the elements top positions are equal, then they are
                        // laid out in a grid-like format)
                ...
                // svElement is the clone. svOriginal is the original element.
                this.$moveUpdate = function(opts, mouse, svElement, svOriginal, svPlaceholder, originatingPart, originatingIndex){
                    ...
                    // opts comes from svElement attribute value ($parse the scope)
                    ...
                    if(!$placeholder){
                        ...
                        // need to hide the original and let the clone take its place when we drag the element
                        ...
                        // onStart is angular.noop function if sv-on-start is not assigned to a function
                        onStart($scope, {
                    ...
                    // * note svRect.width is equal to clientRect.width (theyre the same element)
                    ...
                    // customize helper style
                    $helper[0].style.border = '2px solid #27AE60';

                    // ----- manage candidates
                    getSortableElements(mapKey).forEach(function(se, index){

                        // stop the moving of element if a container is defined and element is approaching outside of the container
                        ...
                        // ** ORIGINAL CODE **/
                        // dragged element will swap with another element the moment mouse cursor hits the outer edge of that element.
                       /* var center = {
                            x: ~~(rect.left + rect.width/2),
                            y: ~~(rect.top + rect.height/2)
                        };*/

                        // MY EDIT - y threshold raised slightly to the top.
                         var center = {
                             x: ~~(rect.left + rect.width/2),
                ...
                this.$drop = function(originatingPart, index, options){
                    ...
                    if(options.revert){
                        ...
                        $helper.css({
                            'top': placeholderRect.top + document.body.scrollTop + 'px',
                            'left': placeholderRect.left + document.body.scrollLeft - $placeholder[0].parent().getBoundingClientRect().left + 'px'
                        });
                    }
                    else
                        afterRevert();
        ...
        return {
            ...
            link: function($scope, $element, $attrs, $controllers){
                ...
                function onMousedown(e){
                    ...
                    // helper is a "handle" that you use to grab onto an element to move around. think door handles
                    if(helper){
                        clone = helper.clone();
                    ...
                    clone[0].reposition = function(coords){
                        ...
                        // Original was causing sortable images in a modal to shoot way off to the right if picked up and dragged. this is because element.style.left only calculates 
                        // from the nearest positioned parent, not body. so we need to subtract the parent's dimensions

                        // MY EDIT
                        this.style.left = targetLeft -clone.parent()[0].getBoundingClientRect().left + 'px';
                        this.style.top = targetTop - angular.element($('.modal-container'))[0].getBoundingClientRect().top+ 'px';
                    };
